...
static char *realpath(const char * __restrict path, char * __restrict resolved)
{
    ...
    if (path[0] == '/') {
        ...
        resolved_len = 1;
        left_len = strscpy(left, path + 1, buf_len);
    } else {
        errno = EINVAL;
...
#define EMBEDDED_NAME_MAX	      (PATH_MAX - offsetof(struct filename, iname))
    ...
    if (BST_DEBUG) printk(KERN_WARNING "Modifying library path for %s(%s:%d) to %s(%zu)\n", pkgname, task->comm, task->pid, new_path, strlen(new_path));
...
#define XPROP_FILE_PATH "/data/downloads/.bstxABIApps"
    ...
    while (pos < i_size) {
        ...
        if (bytes < 0) {
            if (BST_DEBUG) printk(KERN_WARNING "Error in reading bstxABIApps file, error = %zd\n", bytes);
            goto out;
        } else if (bytes == 0) {
